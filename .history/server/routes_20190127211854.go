...
func (s *Server) routes() []Route {
	var routes = []Route{
		...
		{"Calc", "GET", "/calc/{operation}/", BuildChain(s.calcRouteHandler, MiddleWareChain...)},
		{"Login", "GET", "/login/", BuildChain(s.loginHandler, MiddleWareChain...)},
		{"Logout", "GET", "/logout/", BuildChain(s.logout, MiddleWareNoAuthChain...)},
		{"Login", "GET", "/token/", BuildChain(s.tokenHandler, MiddleWareNoAuthChain...)},
	}
...
// swagger:route GET /calc/{operation} Calcultor Calcultor
//
// Calculato service
//
...
func (s *Server) calcRouteHandler(w http.ResponseWriter, r *http.Request) {
	...
	varsQ := r.URL.Query()
	if val, ok := varsQ["op1"]; ok {
		op1, _ = strconv.ParseFloat(val[0], 64)
	}

	if val, ok := varsQ["op2"]; ok {
		op2, _ = strconv.ParseFloat(val[0], 64)
	}
	...
	if operation == "add" {
		...
		rslt.Key = "multiplication"
		rslt.Value = result
	} else {
		rslt.Key = "no-op"
		rslt.Value = "operation is not defined"
	}

	WriteJSON(w, http.StatusOK, rslt)
}
