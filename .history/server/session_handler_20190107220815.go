...
func (s *Server) logout(w http.ResponseWriter, r *http.Request) {
	userJWT := context.Get(r, "uid").(*models.UserJWT)

	sessionCookie, err := services.GetSession(r, "session", userJWT.SessionXID)
	if err == nil {
		services.SaveSessionToStore(r, w, "session", userJWT.SessionXID, 1, sessionCookie)
	} else {
	...
	if err != nil {
		...
		response.WriteJSON(w, http.StatusOK, "User Logged Out")
	}
}