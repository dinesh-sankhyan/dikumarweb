...
// swagger:route GET /calc Health GetHealth
//
// Calculato service
//
...
func (s *Server) calcRouteHandler(w http.ResponseWriter, r *http.Request) {
	var op1, op2 float64
	param1 := mux.Vars(r)["op1"]
	if param1 != "" {
		op1, _ = strconv.ParseFloat(param1, 64)
	}

	param2 := mux.Vars(r)["op2"]
	if param2 != "" {
		op1, _ = strconv.ParseFloat(param2, 64)
	}
	op2 := mux.Vars(r)["op1"]

	varsQ := r.URL.Query()
	operation := "add"
	if val, ok := varsQ["orgId"]; ok {
		operation = val[0]
	}

	result := CalcService.AddValue(op1, op2)
